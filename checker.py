...
def checkReversed(data):
	...
	if bytes == b'\x0F\xAC\xE0\xFF':
		return False
	elif bytes == b'\xAC\x0F\xFF\xE0':
		return True
	bytes = data[0x1C:(0x1C + 0x4)]  # DEADBEEF
	if bytes == b'\xDE\xAD\xBE\xEF':
		return False
	elif bytes == b'\xAD\xDE\xEF\xBE':
		return True
	bytes = data[0x200:(0x200 + 0x4)]  # IFI
	if bytes == b'\x49\x46\x49\x00':
		return False
	elif bytes == b'\x46\x49\x00\x49':
		return True
	bytes = data[0x3F060:(0x3F060 + 0x4)]  # I.DL
	if bytes == b'\x7F\x49\x44\x4C':
		return False
	elif bytes == b'\x49\x7F\x4C\x44':
		return True
...
def isMetldr2(data):
	...
	if bytes == b'\x6D\x65\x74\x6C\x64\x72\x00\x00':   # METLDR
		return "false"
...
def string2hex(data):
	return "".join("{:02x}".format(ord(chr(c))) for c in data)
...
if __name__ == "__main__":
	...
	for node in chktree.findall('.//%s/skumodels/' % flashType):
		...
		if d == skufiledata:
			...
			if node.attrib.get("warn") == "true":
				...
				printcolored("yellow", " %s" % node.attrib.get("warnmsg"))
			break
	if ChkResult == False:
		if risklevel == "DANGER":
			dangerCount += 1
		...
		for subnode in chktree.findall('.//%s/%s/' % (flashType, node.tag)):
			...
			if subnode.tag == "binentry":
				...
				if filedata.lower() == subnode.text.lower():
					...
					if isReversed:
						print("  Actual data (reversed from original) :", end='')
					else:
			...
			if subnode.tag == "multibinentry":
				...
				if ChkResult == False:
					...
					if isReversed:
						...
						print("  Actual data :", end='')
					print_formatedlines(filedata.upper(), 32)
					print("  Expected data (one of the list):", end='')
					for entry in chktree.findall(".//%s/%s/%s[@name='%s']/"%(flashType, node.tag, subnode.tag, subnode.attrib.get("name"))):
	...
	if dangerCount > 0: printcolored("red", dangerCount)
	else: printcolored("green", dangerCount)
	print("Number of warnings =", end=' ')
	if warningCount > 0: printcolored("yellow", warningCount)
	else: printcolored("green", warningCount)
